...
from backend.config import DB_CONFIG, OUTPUT_PATH, EXAMPLE_JSON_PATH
...
def create_des_inputs_from_model_file(
    model_path: str,
    machines: Dict[int, Dict],
    site_name: str,
    sim_time: int = 480,
    ...
    """
    Adapter helper to create DES inputs directly from a model.json file.

    This is a thin wrapper around create_des_inputs that:
    - Loads the model dictionary from JSON on disk
    - Forwards all parameters unchanged

    It is intended for use with model files generated by the
    roads_network_pipeline (MSSM importer) or any other compatible
    model generator.
    """
    with open(model_path, "r", encoding="utf-8") as f:
        model = json.load(f)

    return create_des_inputs(
        model=model,
        machines=machines,
        site_name=site_name,
        sim_time=sim_time,
